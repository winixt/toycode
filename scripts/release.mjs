...
async function publishPackage(pkg, runIfNotDry) {
    ...
    try {
        ...
        console.log(
            ...
            chalk.green(`${pkg.name}@${pkg.newVersion}`),
        );
    } catch (e) {
...
function updateVersions(packagesVersion) {
    ...
    packagesVersion.forEach((p) =>
        updatePackage(p.dirName, p.newVersion, pkgs),
    );
}

const isChangeInCurrentTag = async (pkg, newestTag) => {
    const { stdout: pkgDiffContent } = await run(
        'git',
        ['diff', newestTag, `packages/${pkg}`],
        { stdio: 'pipe' },
    );
...
function genOtherPkgsVersion(packagesVersion) {
    const noChangedPkgs = packages.filter(
        (name) => !packagesVersion.find((item) => item.dirName === name),
    );
    const pkgs = arrToObj(packagesVersion, 'name');
    const result = [];
...
async function main() {
    ...
    const { yes } = await prompt({
        ...
        message: `These packages will be released: \n${packagesVersion
            .map(
                (pkg) =>
                    `${chalk.magenta(pkg.name)}: v${
                        pkg.version
                    } > ${chalk.green(`v${pkg.newVersion}`)}`,
            )
            .join('\n')}\nConfirm?`,
    });
